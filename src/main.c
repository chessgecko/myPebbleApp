...
static char* body_text[1000]; 
int max_length = 12;
int head_char=0;
int space_pos = 0;
int tail_char=1000-1;
bool end = false;
int push_x = 0;
///////////////
  ...
  if(tuple){
    ...
    if(tuple->key == 0){
      APP_LOG(APP_LOG_LEVEL_DEBUG, "Received Message: %s", tuple->value->cstring);
        ...
        APP_LOG(APP_LOG_LEVEL_DEBUG, "1st body_text: %s", *body_text);

        strcat(*body_text, " ");
      
        //APP_LOG(APP_LOG_LEVEL_DEBUG, "2nd body_text: %s", *body_text);
        //if()
        //APP_LOG(APP_LOG_LEVEL_DEBUG, "1st body_text: %s", tuple->value->cstring);
        strcat(*body_text, tuple->value->cstring);
      
        APP_LOG(APP_LOG_LEVEL_DEBUG, "3rd body_text: %s", *body_text);
...
static void getNextWord(char *string[1000], char* word[30]){
  if(end){
    return;
  }
  if(word==NULL){
...
static void change_to_book(){
   ...
   GRect move_pos2 = (GRect) { .origin = { text_x, text_y }, .size = { 144, 168 } };
   layer_set_frame(text_layer_get_layer(display_text),move_pos2);
  /*
   GRect move_pos3 = (GRect) { .origin = { -15, 130 }, .size = { 180, 180 } };
   layer_set_frame(text_layer_get_layer(connection_text),move_pos3);
